...
preloadImages().then(() => {
  ...
  const items = gsap.utils.toArray(paths);

  if (!items.length) return;

  const random = (min, max) => gsap.utils.random(min, max, 0.01);
  ...
  const setGlow = (strength = 0) => {
    const a = 0.18 + strength * 0.10;
    const b = 0.22 + strength * 0.14;
    const c = 0.28 + strength * 0.18;
    const blur1 = 1 + strength * 1.5;
    const blur2 = 3 + strength * 3;
    const blur3 = 6 + strength * 6;

    gsap.set(items, {
      filter: `drop-shadow(0 0 ${blur1}px rgba(255,255,255,${a}))
               drop-shadow(0 0 ${blur2}px rgba(255,255,255,${b}))
               drop-shadow(0 0 ${blur3}px rgba(255,255,255,${c}))`
    });
  };
  ...
  const flickerPath = (path) => {
    const tl = gsap.timeline();

    tl.to(path, {
      opacity: random(0.55, 0.9),
      duration: random(0.015, 0.035)
    })
      .to(path, {
        opacity: random(0.08, 0.18),
        duration: random(0.015, 0.03)
      })
      .to(path, {
  ...
  const startNeonHum = () => {
    ...
    gsap.timeline({
      ...
      .to(glowState, {
        ...
        duration: 0.18,
        ease: "sine.inOut",
        onUpdate: () => setGlow(glowState.strength)
      }, 0)
      .to(items, {
        opacity: () => random(0.96, 0.985),
        duration: () => random(0.12, 0.2),
        stagger: {
          each: 0.01,
          from: "random"
        },
      ...
      .to(items, {
        opacity: () => random(0.985, 1),
        duration: () => random(0.14, 0.22),
        stagger: {
          each: 0.01,
          from: "random"
        },
  ...
  for (let round = 0; round < rounds; round++) {
    shuffle(items).forEach((item) => {
      tl.add(flickerPath(item), `+=${random(0.005, 0.02)}`);
    });
  }

  const finalGlow = { strength: 0 };
  ...
  tl.to(items, {
    ...
    .to(finalGlow, {
      ...
      duration: 0.2,
      ease: "power2.out",
      onUpdate: () => setGlow(finalGlow.strength)
    }, "<")
     .fromTo(".bfg",  {
          y: "120%",
          opacity: 0,
